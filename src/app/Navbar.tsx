    ...
    return (
        <nav className={(isHome) ? ' z-10 fixed w-screen top-0 py-4' : ' z-10 sticky top-0 py-4 bg-white'}>
            ...
            <div className='md:hidden flex justify-center'>
                <div id="works" className={(worksDisplay)}>
                    <ul className={(path === "/") ? 'flex gap-4 flex-col -ml-24' : "flex gap-4 flex-col -ml-14"}>
                        {navbarLinks.works.map((link, idx) => (
                            <li key={idx} className={(link.link === path) ? 'text-zinc-200 duration-200 mix-blend-exclusion bg-blend-luminosity text-ellipsis cursor-pointer' : 'text-zinc-300 hover:text-zinc-200 duration-200 mix-blend-exclusion bg-blend-luminosity text-ellipsis cursor-pointer'}><Link href={link.link}>{link.name}</Link></li>
                        ))}
                    </ul>
                </div>
                <div id="info" className={(infoDisplay)}>
                    <ul className='flex gap-4 flex-col ml-14'>
                        {navbarLinks.info.map((link, idx) => (
                            <li key={idx} className={(link.link === path) ? 'text-zinc-200 duration-200 mix-blend-exclusion bg-blend-luminosity text-ellipsis cursor-pointer' : 'text-zinc-300 hover:text-zinc-200 duration-200 mix-blend-exclusion bg-blend-luminosity text-ellipsis cursor-pointer'}><Link href={link.link}>{link.name}</Link></li>
                        ))}
                    </ul>
